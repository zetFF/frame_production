...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (cursor && cursorDot) {
      ...
      const hoverTargets = document.querySelectorAll("a, button, .morph-card, .float-obj, h1, h2");
      hoverTargets.forEach((el) => {
        el.addEventListener("mouseenter", () => {
          gsap.to(cursor, { scale: 2.5, opacity: 0.6, borderColor: "rgba(168,85,247,0.8)", duration: 0.3, ease: "power2.out" });
        });
        el.addEventListener("mouseleave", () => {
          gsap.to(cursor, { scale: 1, opacity: 1, borderColor: "rgba(255,255,255,0.5)", duration: 0.3, ease: "power2.out" });
        });
      });
    }
    ...
    if (heroSection && heroText) {
      ...
      const handleMouseLeave = () => {
        gsap.to(heroText, { rotateX: 0, rotateY: 0, duration: 1, ease: "elastic.out(1, 0.5)" });
    ...
    if (heroDecor) {
      ...
      gsap.from(heroDecor.querySelectorAll(".hero-corner"), {
        scale: 0, opacity: 0, stagger: 0.1, duration: 1.2, ease: "expo.out", delay: 0.3,
      });

      // Side labels slide in
      gsap.from(heroDecor.querySelectorAll(".hero-side-label"), {
        x: (i) => (i % 2 === 0 ? -40 : 40),
        opacity: 0, stagger: 0.15, duration: 1, ease: "power3.out", delay: 0.6,
      });

      // Tag pills pop in
      gsap.from(heroDecor.querySelectorAll(".hero-tag"), {
        y: 20, opacity: 0, stagger: 0.1, duration: 0.8, ease: "back.out(1.7)", delay: 0.8,
      });

      // Diagonal lines draw in
      gsap.from(heroDecor.querySelectorAll(".hero-line"), {
        scaleX: 0, opacity: 0, stagger: 0.08, duration: 1, ease: "power3.inOut", delay: 0.5,
        transformOrigin: "left center",
      });

      // Floating dots randomize position on scroll
      gsap.from(heroDecor.querySelectorAll(".hero-dot"), {
        scale: 0, opacity: 0, stagger: 0.05, duration: 0.6, ease: "back.out(2)", delay: 1,
      });
      ...
      gsap.from(heroDecor.querySelector(".hero-bottom-bar"), {
        y: 20, opacity: 0, duration: 1, ease: "power3.out", delay: 1.2,
      });
    }
    ...
    gsap.from(hero, { opacity: 0, scale: 0.8, duration: 1.5, ease: "power3.out" });
    ...
    gsap.to(horizontal, {
      ...
      scrollTrigger: {
        ...
        onUpdate: (self) => {
          const fadeStart = 0.8;
          const opacity = self.progress < fadeStart ? 1 : 1 - (self.progress - fadeStart) / (1 - fadeStart);
    ...
    gsap.from(pin, {
      scrollTrigger: { trigger: pin, start: "top center", end: "bottom top", scrub: true },
      opacity: 0, y: 200,
    });
    ...
    if (reveal) {
      ...
      if (image && revealText) {
        gsap.fromTo(reveal,
          ...
          { clipPath: "inset(0% 0% 0% 0% round 0px)", ease: "power2.inOut", scrollTrigger: { trigger: reveal, start: "top bottom", end: "top top", scrub: true } },
        );
        gsap.fromTo(image,
          { scale: 1.3, opacity: 0 },
          { scale: 1, opacity: 1, ease: "none", scrollTrigger: { trigger: reveal, start: "top bottom", end: "top top", scrub: true } },
        );
        ...
        if (overlay) {
          gsap.fromTo(overlay,
            { opacity: 0.7 },
            { opacity: 0.4, ease: "none", scrollTrigger: { trigger: reveal, start: "top top", end: "bottom top", scrub: true } },
          );
        }
      }
      ...
      if (revealDecor) {
        gsap.from(revealDecor.querySelectorAll(".reveal-shape"), {
          scale: 0, opacity: 0, stagger: 0.1, duration: 1, ease: "back.out(1.7)",
        ...
        gsap.to(revealDecor.querySelector(".orbit-ring-2"), {
          rotation: -360, duration: 14, repeat: -1, ease: "none", transformOrigin: "center center",
        });
      }
    }
    ...
    if (split) {
      ...
      split.innerHTML = chars.map((char) =>
    ...
    if (splitDecor) {
      ...
      gsap.from(splitDecor.querySelectorAll(".split-shape"), {
        scale: 0, opacity: 0, rotation: (i) => (i % 2 === 0 ? -30 : 30), stagger: 0.12, duration: 1, ease: "back.out(1.7)",
        scrollTrigger: { trigger: splitSection || splitDecor, start: "top 80%" },
      });
      gsap.from(splitDecor.querySelectorAll(".split-label"), {
        x: (i) => (i % 2 === 0 ? -30 : 30), opacity: 0, stagger: 0.1, duration: 0.9, ease: "power3.out",
        scrollTrigger: { trigger: splitSection || splitDecor, start: "top 75%" },
      });
      gsap.to(splitDecor.querySelectorAll(".split-shape"), {
        y: (i) => (i % 2 === 0 ? -60 : 60), ease: "none",
        scrollTrigger: { trigger: splitSection || splitDecor, start: "top bottom", end: "bottom top", scrub: true },
      });
    }
    ...
    if (parallax) {
      ...
      if (bg && fg) {
        gsap.to(bg, { y: 200, scrollTrigger: { trigger: parallax, start: "top bottom", end: "bottom top", scrub: true } });
        gsap.to(fg, { y: -150, scrollTrigger: { trigger: parallax, start: "top bottom", end: "bottom top", scrub: true } });
      }
    ...
    if (floatingObjs) {
      ...
      items.forEach((item, i) => {
        ...
        gsap.to(item, { y: speed, ease: "none", scrollTrigger: { trigger: floatingObjs, start: "top bottom", end: "bottom top", scrub: true } });
        gsap.to(item, { rotation: i % 2 === 0 ? 45 : -45, ease: "none", scrollTrigger: { trigger: floatingObjs, start: "top bottom", end: "bottom top", scrub: true } });
      });
      const floatTitle = floatingObjs.querySelector(".float-title");
      if (floatTitle) {
        gsap.from(floatTitle, { y: 80, opacity: 0, duration: 1.2, ease: "power3.out", scrollTrigger: { trigger: floatingObjs, start: "top 70%" } });
      }
    }
    ...
    if (counter) {
      ...
      items.forEach((item) => {
        ...
        gsap.to(obj, {
          ...
          scrollTrigger: { trigger: counter, start: "top 75%" },
          onUpdate: () => { numberEl.textContent = Math.round(obj.val).toLocaleString(); },
        });
      });
    }
    ...
    if (marquee) {
      ...
      if (track) {
        ...
        gsap.to([track, clone], { xPercent: -100, repeat: -1, duration: 18, ease: "none" });
        gsap.from(marquee, { opacity: 0, y: 40, duration: 1, scrollTrigger: { trigger: marquee, start: "top 85%" } });
      }
    }
    ...
    if (morph) {
      ...
      gsap.from(Array.from(cards), {
        ...
        scale: 0.5, opacity: 0, rotation: (i) => (i % 2 === 0 ? -15 : 15),
        stagger: { amount: 0.8, from: "center" }, duration: 1, ease: "back.out(1.7)",
      });
      Array.from(cards).forEach((card, i) => {
        gsap.to(card, { y: (i % 3) * 40 - 40, ease: "none", scrollTrigger: { trigger: morph, start: "top bottom", end: "bottom top", scrub: true } });
      });
      const morphTitle = morph.querySelector(".morph-title");
      if (morphTitle) {
        gsap.from(morphTitle, { clipPath: "inset(0% 100% 0% 0%)", duration: 1.2, ease: "power3.inOut", scrollTrigger: { trigger: morph, start: "top 75%" } });
      }
    }
    ...
    if (windowReveal) {
      gsap.fromTo(windowReveal,
        { clipPath: "polygon(0% 0%, 0% 0%, 0% 0%, 0% 0%)" },
        { clipPath: "polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)", ease: "power3.inOut", scrollTrigger: { trigger: windowReveal, start: "top 85%", end: "top 20%", scrub: 1.5 } },
      );
      const inner = windowReveal.querySelector(".window-inner");
      if (inner) {
        gsap.from(inner, { opacity: 0, scale: 0.92, duration: 1.2, ease: "power2.out", scrollTrigger: { trigger: windowReveal, start: "top 50%" } });
      }
    }
    ...
    if (timeline) {
      ...
      if (line) {
        gsap.fromTo(line, { scaleY: 0, transformOrigin: "top center" }, { scaleY: 1, ease: "none", scrollTrigger: { trigger: timeline, start: "top 60%", end: "bottom 80%", scrub: true } });
      }
      const items = timeline.querySelectorAll(".tl-item");
      items.forEach((item, i) => {
        gsap.from(item, { x: i % 2 === 0 ? -100 : 100, opacity: 0, duration: 1, ease: "power3.out", scrollTrigger: { trigger: item, start: "top 80%" } });
        const dot = item.querySelector(".tl-dot");
        if (dot) gsap.from(dot, { scale: 0, duration: 0.5, ease: "back.out(2)", scrollTrigger: { trigger: item, start: "top 80%" } });
      });
      const tlTitle = timeline.querySelector(".tl-title");
      if (tlTitle) gsap.from(tlTitle, { y: 60, opacity: 0, duration: 1, ease: "power3.out", scrollTrigger: { trigger: timeline, start: "top 80%" } });
    }
    ...
    if (footer) {
      ...
      if (bigText) gsap.from(bigText, { y: 120, opacity: 0, duration: 1.4, ease: "power3.out", scrollTrigger: { trigger: footer, start: "top 80%" } });
      gsap.from(Array.from(footer.querySelectorAll(".footer-link")), {
        y: 40, opacity: 0, stagger: 0.08, duration: 0.8, ease: "power3.out", scrollTrigger: { trigger: footer, start: "top 75%" },
      });
      const bottomBar = footer.querySelector(".footer-bottom");
      if (bottomBar) gsap.from(bottomBar, { opacity: 0, y: 20, duration: 1, ease: "power2.out", scrollTrigger: { trigger: footer, start: "top 60%" } });
      footer.querySelectorAll(".footer-line").forEach((line) => {
        gsap.fromTo(line, { scaleX: 0, transformOrigin: "left center" }, { scaleX: 1, duration: 1.2, ease: "power3.inOut", scrollTrigger: { trigger: footer, start: "top 80%" } });
      });
    }
  }, []);
  ...
  return (
    <main className="bg-black text-white overflow-hidden cursor-none">
      {/* CUSTOM CURSOR */}
      <div ref={cursorRef} className="fixed top-0 left-0 w-10 h-10 rounded-full border border-white/50 pointer-events-none z-[9999] mix-blend-difference" style={{ willChange: "transform" }} />
      <div ref={cursorDotRef} className="fixed top-0 left-0 w-2 h-2 rounded-full bg-white pointer-events-none z-[9999] mix-blend-difference" style={{ willChange: "transform" }} />
      ...
      >
        ...
        {/* ── DEKORASI HERO ── */}
        <div ref={heroDecorRef} className="absolute inset-0 pointer-events-none overflow-hidden">
          ...
          {/* === CORNER BRACKETS === */}
          {/* Kiri atas */}
          <div className="hero-corner absolute top-8 left-8 w-10 h-10 border-l-2 border-t-2 border-white/20" style={{ borderRadius: "4px 0 0 0" }} />
          {/* Kanan atas */}
          <div className="hero-corner absolute top-8 right-8 w-10 h-10 border-r-2 border-t-2 border-white/20" style={{ borderRadius: "0 4px 0 0" }} />
          {/* Kiri bawah */}
          <div className="hero-corner absolute bottom-8 left-8 w-10 h-10 border-l-2 border-b-2 border-white/20" style={{ borderRadius: "0 0 0 4px" }} />
          {/* Kanan bawah */}
          <div className="hero-corner absolute bottom-8 right-8 w-10 h-10 border-r-2 border-b-2 border-white/20" style={{ borderRadius: "0 0 4px 0" }} />
          ...
          <div className="hero-line absolute right-0 top-[30%] w-[12%] h-px bg-white/10" style={{ transformOrigin: "right center" }} />
          <div className="hero-line absolute left-0 top-[70%] w-[8%] h-px bg-purple-400/20 origin-left" />
          <div className="hero-line absolute right-0 top-[70%] w-[8%] h-px bg-purple-400/20" style={{ transformOrigin: "right center" }} />

          {/* === TAG PILLS === */}
          {/* Kiri tengah atas */}
          <div className="hero-tag absolute top-[28%] left-16 flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-purple-400/60" />
            <span className="text-[9px] tracking-[0.4em] uppercase text-white/25 font-mono">Multimedia</span>
          </div>
          {/* Kanan tengah atas */}
          <div className="hero-tag absolute top-[28%] right-16 flex items-center gap-2">
            <span className="text-[9px] tracking-[0.4em] uppercase text-white/25 font-mono">Creative</span>
            <div className="w-1.5 h-1.5 rounded-full bg-blue-400/60" />
          </div>
          {/* Kiri tengah bawah */}
          <div className="hero-tag absolute top-[72%] left-16 flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-white/30" />
            <span className="text-[9px] tracking-[0.4em] uppercase text-white/20 font-mono">Visual</span>
          </div>
          {/* Kanan tengah bawah */}
          <div className="hero-tag absolute top-[72%] right-16 flex items-center gap-2">
            <span className="text-[9px] tracking-[0.4em] uppercase text-white/20 font-mono">Studio</span>
            <div className="w-1.5 h-1.5 rounded-full bg-white/30" />
          </div>
          ...
          <div className="absolute top-[75%] right-[22%]">
            <div className="hero-pulse absolute w-3 h-3 rounded-full bg-blue-400/15 -translate-x-1/2 -translate-y-1/2" style={{ animationDelay: "0.6s" }} />
          </div>
          ...
          <div className="hero-corner absolute top-20 left-20 grid grid-cols-3 gap-[8px]">
            {Array.from({ length: 9 }).map((_, i) => (
              <div key={i} className="w-[2px] h-[2px] rounded-full bg-white/15" />
            ))}
          </div>
          <div className="hero-corner absolute bottom-20 right-20 grid grid-cols-3 gap-[8px]">
            {Array.from({ length: 9 }).map((_, i) => (
              <div key={i} className="w-[2px] h-[2px] rounded-full bg-white/10" />
            ))}
          </div>
          ...
          <div className="hero-bottom-bar absolute bottom-16 left-1/2 -translate-x-1/2 flex items-center gap-8">
            ...
            <span className="text-[9px] tracking-[0.5em] uppercase text-white/20 font-mono">Frame Production</span>
            <div className="w-1 h-1 rounded-full bg-purple-400/40" />
            <span className="text-[9px] tracking-[0.5em] uppercase text-white/15 font-mono">Yogyakarta, 2024</span>
            <div className="w-px h-4 bg-white/10" />
          </div>
      ...
      <section className="h-screen flex items-center justify-center">
        <p ref={textRef} className="text-2xl md:text-4xl text-center tracking-tighter max-w-lg">
      ...
      <section className="h-screen overflow-hidden">
        <div ref={horizontalRef} className="flex h-full items-center gap-8 px-20" style={{ width: "max-content" }}>
          {works.map((work) => (
            <div key={work.id} className="relative min-w-[500px] h-[340px] rounded-3xl overflow-hidden group flex-shrink-0">
              <Image
                src={work.image}
                alt={work.title}
                fill
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <h3 className="text-xl font-medium tracking-tight leading-snug">{work.title}</h3>
              </div>
            </div>
          ))}
        </div>
      ...
      >
        <div className="reveal-image absolute inset-0 bg-black w-full h-full" style={{ willChange: "transform, opacity" }} />
        <div className="reveal-overlay absolute inset-0 bg-black/70 z-10" />

        <div ref={revealDecorRef} className="absolute inset-0 z-20 pointer-events-none overflow-hidden">
          <div className="orbit-ring absolute top-[-120px] right-[-120px] w-[420px] h-[420px] rounded-full border border-white/8" style={{ borderStyle: "dashed" }} />
          <div className="orbit-ring-2 absolute bottom-[-80px] left-[-80px] w-[280px] h-[280px] rounded-full border border-purple-400/15" style={{ borderStyle: "dashed" }} />
          <div className="reveal-shape absolute top-10 left-10 grid grid-cols-4 gap-[10px]">
            {Array.from({ length: 16 }).map((_, i) => (<div key={i} className="w-[3px] h-[3px] rounded-full bg-white/20" />))}
          </div>
          <div className="reveal-shape absolute bottom-10 right-10 grid grid-cols-4 gap-[10px]">
            {Array.from({ length: 16 }).map((_, i) => (<div key={i} className="w-[3px] h-[3px] rounded-full bg-white/15" />))}
          </div>
          ...
          <div className="reveal-label absolute bottom-8 left-10 flex flex-col gap-1">
            <span className="text-[10px] text-white/30 tracking-[0.4em] uppercase font-mono">Frame Production</span>
            <span className="text-[10px] text-white/20 tracking-[0.3em] uppercase font-mono">Est. 2024 · Yogyakarta</span>
          </div>
          <div className="reveal-label absolute bottom-8 right-10 flex flex-col items-end gap-1">
            <span className="text-[10px] text-white/30 tracking-[0.4em] uppercase font-mono">Multimedia Studio</span>
            <span className="text-[10px] text-white/20 tracking-[0.3em] uppercase font-mono">✦ Creative · Visual · Live</span>
          </div>
          <div className="reveal-label absolute top-8 left-1/2 -translate-x-1/2">
            <span className="text-[10px] text-white/25 tracking-[0.5em] uppercase font-mono">— Our Services —</span>
          </div>
        ...
        <div className="reveal-text relative z-30 text-center px-10">
          <p className="text-xs tracking-[0.5em] uppercase text-white/40 font-mono mb-6">— Keunggulan Kami —</p>
          <h2 className="text-4xl md:text-7xl font-normal tracking-tight mb-6">Murah Cepat Efisien</h2>
          <p className="text-neutral-300 text-lg md:text-xl max-w-xl mx-auto">memiliki banyak output yang dapat di lihat oleh audiens</p>
          <div className="flex items-center justify-center gap-4 mt-10">
            {["Photography", "Videography", "Livestreaming"].map((tag) => (
              <span key={tag} className="text-[11px] tracking-widest uppercase border border-white/20 rounded-full px-5 py-2 text-white/50 font-mono">{tag}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="h-screen flex items-center justify-center px-10 relative overflow-hidden">
        <div ref={splitDecorRef} className="absolute inset-0 pointer-events-none">
          <div className="split-label absolute left-6 top-1/2 -translate-y-1/2 -rotate-90 origin-center">
            <span className="text-[9px] tracking-[0.6em] uppercase text-white/15 font-mono whitespace-nowrap">Frame Production · Multimedia ·</span>
          </div>
          <div className="split-label absolute right-6 top-1/2 -translate-y-1/2 rotate-90 origin-center">
            <span className="text-[9px] tracking-[0.6em] uppercase text-white/15 font-mono whitespace-nowrap">· Visual · Live · Creative</span>
          </div>
          <div className="split-shape absolute top-[15%] left-[6%] w-10 h-10 border-l-2 border-b-2 border-white/10" style={{ borderRadius: "0 0 0 4px" }} />
          <div className="split-shape absolute bottom-[15%] right-[6%] w-10 h-10 border-r-2 border-t-2 border-white/10" style={{ borderRadius: "0 4px 0 0" }} />
          ...
          <div className="split-label absolute top-[22%] left-1/2 -translate-x-1/2 flex items-center gap-4">
            ...
            <span className="text-[9px] tracking-[0.5em] uppercase text-white/20 font-mono">Portfolio</span>
            <div className="w-12 h-px bg-white/10" />
          </div>
          <div className="split-label absolute bottom-[22%] left-1/2 -translate-x-1/2 flex items-center gap-4">
            <div className="w-8 h-px bg-white/10" />
            <span className="text-[9px] tracking-[0.5em] uppercase text-white/15 font-mono">since 2024</span>
            <div className="w-8 h-px bg-white/10" />
          </div>
          <span className="split-label absolute top-8 left-10 text-[10px] text-white/15 font-mono tracking-widest">03 /</span>
          <span className="split-label absolute top-8 right-10 text-[10px] text-white/15 font-mono tracking-widest">SKILLS</span>
        ...
        <h2 ref={splitTextRef} className="text-5xl md:text-7xl text-center max-w-4xl tracking-tight leading-tight relative z-10">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        <div ref={floatingObjsRef} className="relative w-full h-full">
          ...
          <div className="float-title absolute inset-0 flex flex-col items-center justify-center z-10 text-center px-10">
            <p className="text-neutral-500 text-sm tracking-[0.3em] uppercase mb-4">Fundamental Rancang Art Media Production</p>
            <h2 className="text-5xl md:text-7xl max-w-3xl tracking-tight leading-tight">Apa bukti layanan kami selama ini?</h2>
          </div>
        </div>
      </section>
      ...
      <section className="min-h-screen flex items-center justify-center px-10 py-40">
        <div ref={counterRef} className="w-full max-w-5xl">
          <p className="text-neutral-500 text-sm tracking-[0.3em] uppercase text-center mb-20">Portofolio FRAME PRODUCTION</p>
            ...
            {[
              ...
              <div key={stat.label} className="count-item">
                <div className="flex items-end justify-center gap-1 mb-3">
                  <span className="count-number text-5xl md:text-7xl font-bold tabular-nums" data-target={stat.target}>0</span>
                  <span className="text-3xl md:text-5xl font-bold text-purple-400 mb-1">{stat.suffix}</span>
                </div>
                <p className="text-neutral-500 text-sm tracking-widest uppercase">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 overflow-hidden border-y border-white/5">
        <div ref={marqueeRef} className="flex whitespace-nowrap">
          <div className="marquee-track flex items-center gap-16 pr-16">
            {["Kelompok Bisnis Mahasiswa", "Elrahma Islamic Center", "STMIK El Rahma Yogyakarta", "Van Audio Production"].map((item) => (
              <span key={item} className="text-3xl md:text-5xl font-bold text-white/20 hover:text-white transition-colors duration-300 cursor-default">
                {item}<span className="text-purple-500 mx-8">✦</span>
              </span>
            ))}
          </div>
        </div>
      ...
      <section className="min-h-screen px-10 py-40 overflow-hidden">
        <div ref={morphRef} className="max-w-6xl mx-auto">
          <h2 className="morph-title text-5xl md:text-7xl font-bold mb-20 overflow-hidden tracking-tighter">Capture the Moments</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {[
              { h: "h-[300px]", label: "Identity" }, { h: "h-[500px]", label: "Motion" }, { h: "h-[300px]", label: "Web" },
              { h: "h-[400px]", label: "3D" }, { h: "h-[300px]", label: "Brand" }, { h: "h-[400px]", label: "Digital" },
            ].map((card, i) => (
              <div key={i} className={`morph-card ${card.h} bg-neutral-900 rounded-3xl relative overflow-hidden group cursor-pointer`}>
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <span className="absolute bottom-6 left-6 text-lg font-semibold opacity-0 group-hover:opacity-100 transition-opacity duration-500 translate-y-2 group-hover:translate-y-0 transition-transform">{card.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="h-[150vh] relative flex items-center justify-center overflow-hidden">
        <div ref={parallaxRef} className="w-full h-full relative">
          ...
          <div ref={parallaxDecorRef} className="absolute inset-0 pointer-events-none overflow-hidden">
            ...
            <div className="px-decor-item absolute top-[18%] left-[6%] flex flex-col gap-1">
              <span className="text-[9px] tracking-[0.5em] uppercase text-white/20 font-mono">Depth</span>
            ...
            <div className="px-decor-item absolute bottom-[18%] right-[6%] flex flex-col items-end gap-1">
              ...
              <span className="text-[9px] tracking-[0.5em] uppercase text-white/20 font-mono">Motion</span>
            </div>
            <div className="px-decor-item absolute top-[12%] left-1/2 -translate-x-1/2 grid grid-cols-6 gap-[8px]">
              {Array.from({ length: 12 }).map((_, i) => (<div key={i} className="w-[2px] h-[2px] rounded-full bg-white/15" />))}
            </div>
            <div className="px-decor-item absolute bottom-[12%] left-1/2 -translate-x-1/2 grid grid-cols-6 gap-[8px]">
              {Array.from({ length: 12 }).map((_, i) => (<div key={i} className="w-[2px] h-[2px] rounded-full bg-white/10" />))}
            </div>
            <div className="px-decor-item absolute left-4 top-1/2 -translate-y-1/2 -rotate-90">
              <span className="text-[8px] tracking-[0.7em] uppercase text-white/10 font-mono whitespace-nowrap">Experience · Depth · Visual</span>
            </div>
            <div className="px-decor-item absolute right-4 top-1/2 -translate-y-1/2 rotate-90">
              <span className="text-[8px] tracking-[0.7em] uppercase text-white/10 font-mono whitespace-nowrap">Frame Production · 2024</span>
            </div>
          </div>
          <div className="parallax-fg relative z-10 flex flex-col items-center justify-center h-full gap-6">
            <p className="text-[10px] tracking-[0.6em] uppercase text-white/30 font-mono">— Experience —</p>
            <h2 className="text-5xl md:text-7xl text-center tracking-tighter">Depth & Motion Experience</h2>
            <div className="flex items-center gap-12 mt-4">
              {[{ num: "14+", label: "Projects" }, { num: "3", label: "Clients" }, { num: "142h", label: "Terbang" }].map((s) => (
                <div key={s.label} className="text-center">
                  <div className="text-2xl font-bold text-white/60 tracking-tight">{s.num}</div>
                  <div className="text-[9px] tracking-[0.4em] uppercase text-white/25 font-mono mt-1">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* WINDOW OPEN REVEAL */}
      <div ref={windowRevealRef} className="relative w-full min-h-screen bg-white text-black overflow-hidden" style={{ clipPath: "polygon(0% 0%, 0% 0%, 0% 0%, 0% 0%)" }}>
        <div className="window-inner w-full h-full px-10 md:px-20 py-32 flex flex-col justify-center">
          <p className="text-xs tracking-[0.4em] uppercase text-neutral-400 mb-6">Our Approach</p>
          <h2 className="text-5xl md:text-8xl font-bold leading-tight tracking-tighter max-w-4xl mb-16">We think<br />differently.</h2>
          <div className="grid md:grid-cols-3 gap-10 border-t border-black/10 pt-16">
            {[
              { num: "01", title: "Research First", desc: "Every project begins with deep understanding — of your audience, your goals, and the landscape." },
              { num: "02", title: "Design with Intent", desc: "No decoration for its own sake. Every element serves a purpose, drives a feeling, creates meaning." },
              { num: "03", title: "Build to Last", desc: "We craft experiences that age well — technically sound, performant, and built for humans." },
            ].map((col) => (
              <div key={col.num}>
                <span className="text-xs text-neutral-400 mb-4 block">{col.num}</span>
                <h3 className="text-xl font-bold mb-3 tracking-tighter">{col.title}</h3>
                <p className="text-neutral-500 text-sm leading-relaxed">{col.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="min-h-screen bg-white px-10 md:px-20 py-40 overflow-hidden">
        <div ref={timelineRef} className="max-w-4xl mx-auto">
          <p className="text-neutral-400 text-xs tracking-[0.4em] uppercase mb-4">Our Process</p>
          <h2 className="tl-title text-5xl md:text-7xl font-bold tracking-tighter text-black mb-24">How we work</h2>
          <div className="relative">
            <div className="tl-line absolute left-1/2 -translate-x-1/2 w-px h-full bg-black/10 origin-top" />
            {[
              { year: "Week 01–02", title: "Discovery", desc: "Deep-dive workshops, stakeholder interviews, competitive analysis, and defining success metrics.", side: "left" },
              { year: "Week 03–04", title: "Strategy", desc: "Information architecture, user flows, content strategy, and technical scoping.", side: "right" },
              { year: "Week 05–07", title: "Design", desc: "Wireframes to high-fidelity. Motion principles, design system, and interactive prototypes.", side: "left" },
              { year: "Week 08–11", title: "Build", desc: "Component-driven development, CMS integration, performance optimization.", side: "right" },
              { year: "Week 12", title: "Launch", desc: "QA, accessibility audit, analytics setup, staged rollout, and post-launch monitoring.", side: "left" },
            ].map((item, i) => (
              <div key={i} className={`tl-item relative flex items-start mb-20 ${item.side === "left" ? "flex-row pr-[55%]" : "flex-row-reverse pl-[55%]"}`}>
                ...
                <div className={item.side === "right" ? "text-right" : "text-left"}>
                  <span className="text-xs text-purple-400 tracking-widest block mb-2">{item.year}</span>
                  <h3 className="text-2xl font-bold mb-2 text-black tracking-tighter">{item.title}</h3>
                  <p className="text-neutral-500 text-sm leading-relaxed max-w-xs">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* FOOTER */}
      <footer ref={footerRef} className="relative bg-black text-white px-10 md:px-20 pt-32 pb-12 overflow-hidden">
        ...
        <div className="overflow-hidden mb-20">
          <h2 className="footer-big-text text-6xl md:text-[10vw] font-bold leading-none tracking-tighter">
            ...
            <span className="text-transparent" style={{ WebkitTextStroke: "1px rgba(255,255,255,0.3)" } as React.CSSProperties}>extraordinary.</span>
          </h2>
        </div>
        <div className="mb-24">
          <a href="mailto:hello@frameproduction.com" className="footer-link inline-flex items-center gap-4 border border-white/20 rounded-full px-8 py-4 text-sm tracking-widest uppercase hover:bg-white hover:text-black transition-all duration-500 group">
            Start a Project
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-12 mb-20">
          <div className="col-span-2 md:col-span-1">
            <p className="text-lg font-bold tracking-tighter mb-4">FRAME PRODUCTION</p>
            <p className="text-neutral-600 text-sm leading-relaxed tracking-tighter max-w-xs">We design and build digital experiences that move people — literally and emotionally.</p>
          </div>
          <div>
            <p className="text-xs tracking-[0.3em] uppercase text-neutral-600 mb-6">Work</p>
            <ul className="space-y-3">
              {["Projects", "Case Studies", "Selected Works", "Awards"].map((item) => (
                <li key={item}><a href="#" className="footer-link text-sm text-neutral-400 hover:text-white transition-colors duration-300">{item}</a></li>
              ))}
            </ul>
          </div>
          <div>
            <p className="text-xs tracking-[0.3em] uppercase text-neutral-600 mb-6">Studio</p>
            <ul className="space-y-3">
          ...
          <div>
            <p className="text-xs tracking-[0.3em] uppercase text-neutral-600 mb-6">Social</p>
            <ul className="space-y-3">
              {["Instagram", "Twitter / X", "LinkedIn", "Dribbble"].map((item) => (
                <li key={item}>
                  <a href="#" className="footer-link text-sm text-neutral-400 hover:text-white transition-colors duration-300 inline-flex items-center gap-2 group">
                    {item}<span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-purple-400 text-xs">↗</span>
                  </a>
                </li>
              ))}
            </ul>
        ...
        <div className="footer-bottom flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <p className="text-neutral-700 text-xs tracking-widest uppercase">© 2025 Frame Production. All rights reserved.</p>
          <div className="flex items-center gap-8">
            <a href="#" className="text-neutral-700 text-xs tracking-widest uppercase hover:text-white transition-colors duration-300">Privacy Policy</a>
            <a href="#" className="text-neutral-700 text-xs tracking-widest uppercase hover:text-white transition-colors duration-300">Terms of Use</a>
            <span className="text-neutral-700 text-xs tracking-widest uppercase">Jakarta, Indonesia 🇮🇩</span>
          </div>
        </div>
      </footer>
    </main>
